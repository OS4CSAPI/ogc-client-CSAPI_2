...
> **~~Implementation Gap~~ RESOLVED: `latest` Keyword Now Supported (Issue #43)**
>
> Fixed in Issue #43. The CSAPI-local type `CsapiDateTimeParameter = DateTimeParameter | 'latest'` was added to `model.ts`, keeping the `'latest'` keyword scoped to CSAPI without modifying the shared `DateTimeParameter` used by EDR. The `formatDateTimeParameter()` function in `helpers.ts` now passes through the `'latest'` string. The `resultTime` fields in `DatastreamQueryOptions` and `ObservationQueryOptions` use the extended type.
>
> Confirmed working against OSH (smoke test F19: `resultTime=latest` returns 200). 3 new tests added (245 total).
