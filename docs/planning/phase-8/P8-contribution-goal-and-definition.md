...
| A4  | Validators in `csapi/` throw `EndpointError` (or a CSAPI-specific subclass), never plain `Error` / `TypeError`  | `git grep "throw new Error\|throw new TypeError" -- src/ogc-api/csapi/` returns 0 matches; factory wraps thrown errors uniformly              |
...
- **Finding 021 — Validators throw plain `Error`; factory propagates `TypeError`.** Standardize on `EndpointError` (or a CSAPI-specific subclass to be agreed during implementation). All `throw new Error(...)` / `throw new TypeError(...)` in `csapi/` become `throw new EndpointError(...)`. The factory wraps any error thrown during init in a CSAPI-typed error so consumers can `instanceof`-check uniformly.
