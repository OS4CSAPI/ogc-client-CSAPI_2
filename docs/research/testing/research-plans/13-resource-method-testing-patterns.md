...
**Priority:** HIGH - Consistent pattern across all resource types
...
**Consistency Need:** With 9 resource types, need consistent testing approach to:
- **Avoid duplication:** Don't write same tests 9 times
- **Ensure completeness:** All resource types get equal coverage
- **Maintain quality:** Uniform "meaningful" test depth
- **Enable reuse:** Template/pattern that scales

This research defines:
- **CRUD Pattern:** Consistent testing for Create, Read, Update, Delete operations
- **Query Pattern:** Consistent testing for collection queries
- **Template:** Reusable test structure applicable to all resource types
- **Differentiation:** What's unique per resource type vs shared
- **Test Depth:** What constitutes "meaningful" resource method testing
...
1. What CRUD operations exist per resource type?
2. Are all operations supported for all resource types?
...
6. What's consistent across all `getSomethings()` methods?
7. What query parameters are common (limit, offset, bbox)?
8. What query parameters are resource-specific?
...
16. What error conditions (404, etc.)?
...
22. What error conditions (400, 409, etc.)?
...
39. What error conditions (404, 409, etc.)?

### Resource-Specific Testing

40. **Systems:** What's unique to Systems resource methods?
41. **Deployments:** What's unique to Deployments resource methods?
42. **Procedures:** What's unique to Procedures resource methods?
43. **SamplingFeatures:** What's unique to SamplingFeatures resource methods?
44. **Properties:** What's unique to Properties resource methods?
45. **DataStreams:** What's unique to DataStreams resource methods?
46. **Observations:** What's unique to Observations resource methods?
47. **ControlStreams:** What's unique to ControlStreams resource methods?
48. **Commands:** What's unique to Commands resource methods?

### Query Parameters by Operation

49. What query parameters apply to GET collections?
50. Do POST/PUT/PATCH have query parameters?
51. What temporal parameters exist per resource type?
52. What spatial parameters exist per resource type?
53. What filtering parameters exist per resource type?
...
60. What status codes are expected per operation?
...
66. GET item: What errors are possible (404, etc.)?
67. POST create: What errors are possible (400, 409, etc.)?
68. PUT update: What errors are possible (404, 400, etc.)?
69. PATCH update: What errors are possible?
70. DELETE: What errors are possible (404, 409, etc.)?

### Template Design

71. What describe block structure scales across 9 resource types?
72. What test utilities can be shared?
73. What fixtures can be reused?
74. How to parametrize tests for multiple resource types?
75. What's the test-to-operation ratio?
...
1. Review Implementation Guide resource method specifications
2. Document CRUD operations per resource type
3. Identify common patterns across operations
4. Identify resource-specific variations
5. Create CRUD operation matrix

### Phase 2: Upstream Resource Method Analysis (20-30 minutes)

1. Review Section 1-2 deliverables (upstream patterns)
2. Analyze how upstream tests resource methods
3. Extract assertion patterns
4. Document test utilities
5. Create resource method testing best practices

### Phase 3: Template Design (30-45 minutes)

1. Design consistent test structure for all resource types
2. Define shared test utilities
3. Design fixture organization
4. Create test template with placeholders
5. Document how to apply template to each resource type

### Phase 4: Resource-Specific Differentiation (20-30 minutes)

1. Document what's unique per resource type
2. Define how to extend template for uniqueness
3. Create resource-specific test addendum examples
4. Document fixture requirements per resource type
...
- **Section 12 Deliverable:** QueryBuilder testing strategy (methods use QueryBuilder)
- **Section 8 Deliverable:** CSAPI specification requirements (resource operation specs)
...
- Section 6 Deliverable: "Meaningful vs Trivial" Definition
...
### Document: "Resource Method Testing Template"

**Location:** `docs/research/testing/results/13-resource-method-testing-template.md`

**Note:** This goes in `results/` because it's a synthesized, actionable template (not raw findings).
...
- Estimated test lines per resource type
- Test organization strategy

#### 2. CRUD Operations Matrix

```markdown
| Resource Type | GET Collection | GET Item | POST Create | PUT Update | PATCH Update | DELETE | Priority |
|---------------|----------------|----------|-------------|------------|--------------|--------|----------|
| Systems | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | CRITICAL |
| Deployments | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | HIGH |
| Procedures | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | MEDIUM |
| SamplingFeatures | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | HIGH |
| Properties | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | HIGH |
| DataStreams | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | CRITICAL |
| Observations | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | CRITICAL |
| ControlStreams | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | MEDIUM |
| Commands | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ | MEDIUM |
```

#### 3. Universal Test Template

**Template Structure (Apply to Each Resource Type):**
...
describe('ResourceType Methods', () => {
  ...
  describe('getResourceTypes()', () => {
    it('should build collection URL with no parameters', () => {
      const url = builder.getResourceTypes();
      validateUrl(url, {
        pathname: '/collections/collection-id/resourcetypes',
        query: {}
      });
    });
    
    it('should build collection URL with pagination parameters', () => {
      const url = builder.getResourceTypes({ limit: 10, offset: 20 });
      validateUrl(url, {
        pathname: '/collections/collection-id/resourcetypes',
    ...
    it('should build collection URL with filtering parameters', () => {
      const url = builder.getResourceTypes({ 
        bbox: [-180, -90, 180, 90],
        // resource-specific filter (e.g., systemType for Systems)
      });
      validateUrl(url, {
        pathname: '/collections/collection-id/resourcetypes',
        query: { bbox: '-180,-90,180,90', /* resource-specific */ }
      });
    });
    
    // Resource-specific query parameter tests go here
  });
  ...
  describe('getResourceType(id)', () => {
    it('should build item URL with valid ID', () => {
    ...
    it('should encode special characters in ID', () => {
      const url = builder.getResourceType('resource with spaces');
      validateUrl(url, {
        pathname: '/collections/collection-id/resourcetypes/resource%20with%20spaces',
        query: {}
      });
    });
  });
  ...
  describe('createResourceType(body)', () => {
    it('should build create URL', () => {
      const url = builder.createResourceType({
        uniqueIdentifier: 'https://example.com/resource-123',
    ...
    // Validate request body structure (if applicable to testing approach)
  });
  ...
  // PATCH UPDATE TESTS
  // ====================
  describe('patchResourceType(id, body)', () => {
    it('should build patch URL', () => {
      const url = builder.patchResourceType('resource-123', {
