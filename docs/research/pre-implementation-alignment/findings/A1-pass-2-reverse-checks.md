...
Pass 2 executes the four reverse checks from the A1 research plan, asking: *"Has the test research discovered anything the implementation guide should incorporate?"*
...
| §7 Validator | Part 1/Part 2 validation rules | ⚠️ **Ambiguous** | Some rules validate *server response correctness* (e.g., "uniqueIdentifier must be valid URI", "systemType must be from SOSA vocabulary"). Client should extract these values, not validate vocabulary membership |
...
**Severity: Low.** The §7 Validator section is about *what the validator component will do* (implementation specification), not about test design. The risk is that someone might write tests that assert server data values match these rules (AP1/AP4), but the guide itself doesn't propose that.
...
*Generated by A1 Pass 2 execution. Next: A1 Pass 3 (Checks 9-12, Bidirectional + Final Report Assembly).*
