...
6. **Check 6 (Omissions):** Read Implementation Guide table of contents/section headers. Identify major scope areas NOT mentioned in the Contribution Goal:
   - Worker Extensions (§8) — mentioned? **Note:** Worker extensions are now OUT OF SCOPE (ROADMAP v3.1). If the Contribution Goal doesn't mention them, that's correct. But verify the Contribution Goal doesn't *imply* worker support.
