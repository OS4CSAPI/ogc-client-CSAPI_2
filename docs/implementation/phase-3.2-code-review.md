...
**Commits:**
- `fc4c90c` — docs: add user-discussion gate to Phase 3 smoke test template *(doc-only)*
- `e7b07c7` — docs: integrate Phase 3.1 smoke test findings F40–F46 into ROADMAP v3.4 *(doc-only)*
...
```typescript
if (sErr.length === 0 && eErr.length === 0 &&
...
it('reports end before start', () => {
  const errors = validateTimePeriod(
    ['2027-01-01T00:00:00Z', '2026-01-01T00:00:00Z'], 'tp'
  );
...
it('accepts resultSchema as alternative to schema', () => {
  expect(validateDatastream({ name: 'Weather', resultSchema: { type: 'DataRecord' } }))
    .toEqual([]);
});
```
