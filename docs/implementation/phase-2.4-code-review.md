...
**Status:** Unchanged. Still uses `toContain('datetime=')` instead of an exact `toBe()`. Now the *only* `toContain` assertion in the entire url_builder spec (verified via grep — 1 occurrence at line 751). The contrast with the SamplingFeatures datetime test (line ~1127, exact `toBe()`) makes this more conspicuous.
